...
/**
	...
	Helper class from practical lessons.
	Used for the loading of .obj models and giving it the correct textures
...
*/
ObjModel::ObjModel(const std::string& fileName, glm::vec3 pos)
{
	position = pos;
...
void ObjModel::draw()
{
	...
	{
		if (this->materials[group->materialIndex]->texture != nullptr)
		{
			this->materials[group->materialIndex]->texture->bind(); // if there is a material, bind it, else no binding occurs
			tigl::shader->enableTexture(true); // and enable texture drawing
		}
		else
		{
			tigl::shader->enableTexture(false); // else disable texture drawing
		}
		...
		for (auto& face : group->faces)
		{
			...
			{
				tigl::addVertex(tigl::Vertex::PTN(this->vertices[vertex.position] + position, this->texcoords[vertex.texcoord], 
					this->normals[vertex.normal]));
			}
		}
		tigl::end();
	}
...
{
	//std::cout << "\033[A\33[2K\rLoading " << fileName << std::endl;
	std::ifstream pFile(fileName.c_str());
	if (!pFile.is_open())
	{
		std::cout << "Could not open file " << fileName << std::endl;
